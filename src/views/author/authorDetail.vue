          ...
          <el-tabs v-model="activeName" style="text-align:center" @tab-click="handleTabClick">
            <el-tab-pane label="推荐文章" name="recommend">
              <div class="recommend-list">
                ...
                <p class="waiting">
                  <span v-if="recommendWaiting">努力加载中...</span>
            ...
            <el-tab-pane label="发布文章" name="publish">
              <div class="publish-list">
                ...
                <p class="waiting">
                  <span v-if="publishWaiting">努力加载中...</span>
                  <span v-else>没有更多了...</span>
                </p>
              </div>
            </el-tab-pane>
            <el-tab-pane label="点赞文章" name="like">
              <div class="like-list">
                ...
                <p class="waiting">
                  <span v-if="likeWaiting">努力加载中...</span>
                  <span v-else>没有更多了...</span>
                </p>
              </div>
            </el-tab-pane>
                ...
                <div class="follow-item item" v-for="(item, index) in followList" :key="item.id">
                  ...
                  <div class="item-info">
                    <div class="author-username">
                      <p class="text">
                        <router-link :to="'/author/detail?user='+item.username">{{ item.username }}</router-link>
                      </p>
                    </div>
                  ...
                  <div class="item-operate">
                    <span
                      v-if="!item.is_follow"
                      class="follow-btn"
                      @click="handleFollowAuthor(item, index, 'follow')"
                    >关注</span>
                    <span
                      v-else
                      class="unfollow-btn"
                      @mouseover="handleToggleText(item, index, 'follow')"
                      @mouseout="handleToggleText(item, index, 'follow')"
                      @click="handleFollowAuthor(item, index, 'follow')"
                    >{{ item.is_hover ? '取消关注' : '已关注' }}</span>
                  </div>
                </div>
                <el-pagination
                ...
                >
                  ...
                  <div class="item-operate">
                    <span
                      v-if="!item.is_follow"
                      class="follow-btn"
                      @click="handleFollowAuthor(item, index, 'followed')"
                    >关注</span>
                    <span
                      v-else
                      class="unfollow-btn"
                      @mouseover="handleToggleText(item, index, 'followed')"
                      @mouseout="handleToggleText(item, index, 'followed')"
                      @click="handleFollowAuthor(item, index, 'followed')"
                    >{{ item.is_hover ? '取消关注' : '已关注' }}</span>
                  </div>
                </div>
  ...
  methods: {
    ...
    handleCropperImage() {
      this.$refs.cropper.getCropData(data => {
        ...
        this.$axios
          .post('/account/api/upload/author_bg', formData)
          .then(res => {
            if (res) {
              this.dialogEditImage = false
    ...
    handleScroll() {
      ...
      this.timeout = setTimeout(() => {
        ...
        if (bottomOfWindow < 20) {
          if (
            this[this.activeName + 'Page'] < this[this.activeName + 'Total']
          ) {
    ...
    handleFollowAuthor(item, index, type) {
      //关注或者 取消关注
