       ...
       <div class="column-table table">
           <el-table :data="columnList">
               ...
               <el-table-column label="状态">
                   <template v-slot="scope">
                       <div>
                           {{ scope.row.status === 1 ? '发布' : '未发布' }} (是否显示在个人主页)
                       </div>
                   </template>
               </el-table-column>
               <el-table-column label="操作">
                   <template v-slot="scope">
                       <div>
                           <span class="up" v-if="scope.row.status === 0" @click="handlePubColumn(scope.row.id, scope.row.columnName)">发布</span>
                           <span class="down" v-else @click="handleDownColumn(scope.row.id, scope.row.columnName)">不显示</span>
...
export default {
    ...
    methods: {
        handleSearchColumn() { //搜索个人专栏

        },
        handleJumpAddColumn() { // 跳转到新增专栏页面
            this.$router.push('add');
        },
        indexMethod( index) { //table 序号
            return index + 1 +(this.currentPage - 1)*this.size;
        },
        handlePubColumn(id, columnName){ //发布
            console(id,columnName)
        },
        handleDownColumn(id, columnName){ //不显示在主页

        },
        handleDelColumn(id, columnName){ //删除column
            this.$confirm('确认删除个人专栏 -- 《'+columnName+"》 ？ 该专栏下的文章也会删除 !", '提示', {
                confirmButtonText: '确认',
                cancelButtonText: '取消',
                type: "warning",
                center: 'true' 
            }).then(() => {
                this.$axios.post('/article/api/del/personal_column',
                    qs.stringify({
                        columnId: id
                    })
                ).then(res => {
        ...
        handleGetColumnList() {
            this.$axios.post('/article/api/get/personal_column?page='+this.currentPage+'&size='+this.size,
                qs.stringify({
                    status: this.status,
