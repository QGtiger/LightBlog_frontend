    ...
    <div class="article-form">
      ...
      <div class="mavon-editor">
        <mavon-editor
          ...
          v-model="articleForm.bodyHtml"
          ref="editor"
          @imgAdd="handleAddImg"
          @imgDel="handleDelImage"
          @save="handleSave"
        ></mavon-editor>
...
export default {
  ...
  data() {
    var checkTitleName = (rule, value, callback) => {
      ...
      setTimeout(() => {
        if (value.length > 250) {
          callback(new Error('标题长度最多250个字符'))
  ...
  methods: {
    ...
    handleBeforeUpload(file) {
      ...
      if (!isLt4M) {
        this.$message.error('选择图片大小不能超过 4MB!')
      }
      return isLt4M && isImg
    },
    ...
    handleGetColumnTheme() {
      this.$axios.post('/article/api/get/columnTheme').then(res => {
        if (res) {
          ...
          this.articleForm.columnId =
            this.$route.query.columnId === undefined
              ? this.articleForm.columnId
              : parseInt(this.$route.query.columnId)
          this.articleForm.themeId =
            this.$route.query.themeId === undefined
              ? this.articleForm.themeId
              : parseInt(this.$route.query.themeId)
        }
      })
    },
    handleGetColumnList() {
      this.$axios.post('/article/api/get/personal_column').then(res => {
        if (res) {
    ...
    handlePublishArticle() {
      ...
      this.$refs.articleForm.validate(valid => {
        if (valid) {
          if (this.isUpdate) {
            if (
              this.articleForm.bodyHtml.replace(/(^\s*)|(\s*$)/g, '') === ''
            ) {
            ...
            let coverImageBlob =
              this.isUpdateImg === 1
                ? this.handleToBlob(this.articleForm.previewImg[0].url)
                : ''
            const postData = {
              id: this.updateId,
            ...
            this.$axios
              .post('/article/api/update/article', formData)
              .then(res => {
                if (res) {
            ...
            this.$axios
              .post('/article/api/publish/article', formData)
              .then(res => {
                if (res) {
                  this.$message.success('发布成功')
                  this.isUpdate = true
    ...
    handleAddImg(pos, $file) {
      ...
      this.$axios.post('/article/api/upload/image', formData).then(res => {
