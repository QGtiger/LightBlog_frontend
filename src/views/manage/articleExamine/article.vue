...
<template>
  <div class="article-examine">
      ...
      <div class="table-cont table">
        <el-table :data="articleList">
          ...
          <el-table-column label="审核时间">
            <template
              slot-scope="scope"
            >{{ scope.row.checked ? $util.Time.getAllTime(scope.row.checked):'--' }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作">
            <template v-slot="scope">
              <div>
                ...
                <span
                  class="detail"
                  @click="handleGetDetailCheck(scope.row.id)"
    ...
    <el-drawer title="审核结果" :visible.sync="drawerShowResult" direction="ltr" size="30%">
      <div class="drawer-result">
        <el-form :model="resultForm" ref="resultForm" label-width="100px" label-position="left">
          <el-form-item label="文章标题">{{ resultForm.title }}</el-form-item>
          <el-form-item label="文章状态">
            <span>{{ resultForm.status == 2 ? '已驳回' : '已通过' }}</span>
          </el-form-item>
          <el-form-item label="文章简介">{{ resultForm.description }}</el-form-item>
          <el-form-item label="是否推荐">
            <span>{{ resultForm.isRecommend ? '推荐' : '不推荐' }}</span>
          </el-form-item>
          <el-form-item label="审核回复内容">{{ resultForm.content }}</el-form-item>
        </el-form>
      </div>
    </el-drawer>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleGetArticleList() {
      ...
      this.$axios
        .post(
          `/article/api/admin/get/articlelist?page=${this.currentPage}&size=${this.size}`,
          qs.stringify({
    ...
    handleJumpExamine(id) {
      //跳转审核页面
      this.$store.commit('setSearchHref', this.searchHref)
      this.$store.commit('setSearchData', this._data)
      this.$router.push({
        path: 'check',
        query: {
          id
    ...
    handleGetDetailCheck(id) {
      //查看申请详情
      this.$axios
        .post(
          '/article/api/get/articlecheck',
          qs.stringify({
            id
          })
        )
  ...
  created() {
    this.searchHref = this.$route.path
    if (
      this.$store.state.searchHref === this.$route.path &&
      this.$store.state.searchData != ''
    ) {
      console.log(123)
      this._data = this.$store.state.searchData
    }
    this.$store.commit('setSearchHref', '')
