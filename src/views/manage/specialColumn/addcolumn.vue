...
<template>
  <div class="add-column-cont">
    ...
    <div class="form-cont">
      <el-form
        :model="specialColumnForm"
        ref="specialColumnForm"
        label-width="100px"
        :rules="rules"
        label-position="left"
      >
...
export default {
  ...
  methods: {
    ...
    handleBeforeUpload(file) {
      //上传图片格式限制
      const isLM4M = file.size / 1024 / 1024 < 4
    ...
    hanldSubmit() {
      this.$refs.specialColumnForm.validate(valid => {
        if (valid) {
          if (this.isUpdate) {
            let coverImageBlob = this.isUpdateImage
              ? this.handleToBlob(this.specialColumnForm.coverUrl[0].url)
              : ''
            ...
            this.$axios
              .post('/article/api/update/special_column', formData)
              .then(res => {
            ...
            const postData = {
              ...
              cover_image: this.handleToBlob(
                this.specialColumnForm.coverUrl[0].url
              )
            }
            Object.keys(postData).forEach(key => {
              formData.append(key, postData[key])
            ...
            this.$axios
              .post('/article/api/add/special_column', formData)
              .then(res => {
                this.$message.success(res.data.tips)
                this.specialColumnForm = {
                  columnName: '',
    ...
    handleGetColumnData() {
      ...
      this.$axios
        .post(
          '/article/api/detail/special_column',
