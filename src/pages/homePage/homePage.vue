    ...
    <div class="blog-main-cont">
      ...
      <div class="recommendSpecialColumn">
        <div class="title">
          <p class="title-text">
            推荐专栏
            <span class="tips">知识体系专栏，可通过专栏去系统学习某个知识点</span>
            ...
            <div class="blog-list">
              <div class="blog-item" v-for="item in recommendList" :key="item.id">
                ...
                <div class="item-content">
                  ...
                  <p class="item-date">
                    {{ $util.Time.getFormatTime(item.updated) }}
                    <span
                      class="scanCount"
                    >浏览 {{item.scanCount}} 次</span>
                  </p>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    handleGetRecommendBlogs() {
      this.$axios
        .post(
          '/article/api/get/home/articles?page=' +
            this.currentPage +
            '&&size=' +
            this.size
        )
