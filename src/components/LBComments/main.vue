      ...
      >
        <div class="LBComment--RootComment">
          <comments-item
            ...
            :comment="item.comment_root"
          ></comments-item>
        </div>
        <div class="child-comment-items">
          <div
            class="LBComment--ChildComment"
            v-for="reply in item.comment_reply"
            :key="reply.id"
          >
            <reply-item
        ...
        <div class="more_reply">
          <span
            v-if="item.comment_reply_more"
            @click="handleGetMoreReply(item.comment_root.id, index)"
            >查看全部 {{ item.total_comment_reply }} 条评论</span
          >
        </div>
      </div>
    </transition-group>
...
export default {
  ...
  methods: {
    ...
    handleGetMoreReply(id, index) {
      this.$axios
        .post(
          '/comment/api/comment_reply/more',
          this.$qs.stringify({
            commentId: id
          })
        )
